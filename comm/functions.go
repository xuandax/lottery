...
func FormatFromUnixTime(t int64) string {
	if t > 0 {
		return time.Unix(t, 0).Format(conf.SystimeFormat)
	} else {
		return time.Now().Format(conf.SystimeFormat)
	}
}
...
func FormatFromUnixTimeShort(t int64) string {
	if t > 0 {
		return time.Unix(t, 0).Format(conf.SystimeFormatShort)
	} else {
		return time.Now().Format(conf.SystimeFormatShort)
	}
}

// 将字符串转成时间
func ParseTime(str string) (time.Time, error) {
	return time.ParseInLocation(conf.SystimeFormat, str, conf.SysTimeLocation)
}
